...
class PyMongoUtils:
    ...
    @staticmethod
    def update_player_hof(player: Player, database: Database):
    ...
    @staticmethod
    def insert_hof_class(player: Player, database: Database):
    ...
    @staticmethod
    def insert_awards(awards: Awards, database: Database):
    ...
    @staticmethod
    def insert_season_to_awards(season: Season, awards_id, season_id, database: Database):
    ...
    @staticmethod
    def insert_team_player_season(team: Team, player_id: str, season_id: str, database: Database):
    ...
    def upsert_batting_stats(batting_stats_obj: BattingStats, player_id: str, season_id: str, database: Database):
        ...
        batting_stats_model = batting_stats_obj.to_model()
    ...
    def insert_season(season: Season, database: Database):
        ...
        season_model = season.to_model(playoffs, regular_season)
    ...
    @staticmethod
    def get_all_player_pitching(player_id, database: Database):
        pitching_collection = database.get_collection(PYMONGO_PITCHING_STATS_COLLECTION)
        all_stats = pitching_collection.find({PYMONGO_PLAYER : player_id, PYMONGO_TEAM : None})
        return all_stats

    @staticmethod
    def get_all_player_batting(player_id, database: Database):
        batting_collection = database.get_collection(PYMONGO_BATTING_STATS_COLLECTION)
        all_stats = batting_collection.find({PYMONGO_PLAYER : player_id, PYMONGO_TEAM : None})
        return all_stats
    
    @staticmethod
    def insert_seasons_awards(awards: Awards, season_id, database: Database):
        awards_id = PyMongoUtils.insert_awards(awards, database)
        PyMongoUtils.insert_season_award(awards_id, season_id, database)
    
    @staticmethod
    def update_players_hof(players: list[Player], database):
        for player in players:
